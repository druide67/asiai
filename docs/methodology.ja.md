...
**Ollama**：`prompt_eval_duration`（内部タイミング）によりサーバーサイドで測定。ネットワークオーバーヘッドのない純粋なプロンプト処理時間です。`ttft_source: server` として報告。

**OpenAI互換エンジン**：最初のSSEコンテンツチャンクでクライアントサイドで測定。HTTPセットアップ、リクエスト送信、サーバー処理を含みます。通常サーバーサイドより10-100ms高くなります。`ttft_source: client` として報告。

!!! warning "TTFTの比較"
    差異を考慮せずに、OllamaのサーバーサイドTTFTとOpenAI互換エンジンのクライアントサイドTTFTを比較しないでください。ベンチマーク結果の `ttft_source` フィールドが、どの方法が使用されたかを示しています。
