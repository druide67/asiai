...
<tbody id="lb-body">
<tr><td colspan="7" class="lb-loading">コミュニティデータを読み込み中...</td></tr>
</tbody>
</table>
  ...
  function fetchData() {
    ...
    fetch(url)
      ...
      .then(function(d) {
        ...
        render();
        document.getElementById("lb-count").textContent = data.length + " 件の結果";
        document.getElementById("lb-updated").textContent =
          "更新: " + new Date().toLocaleTimeString();
      })
      .catch(function() {
        setMessage("api.asiai.dev に接続できませんでした");
      });
  }
