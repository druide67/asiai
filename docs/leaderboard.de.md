...
<tbody id="lb-body">
<tr><td colspan="7" class="lb-loading">Community-Daten werden geladen...</td></tr>
</tbody>
</table>
  ...
  function fetchData() {
    ...
    fetch(url)
      ...
      .then(function(d) {
        ...
        render();
        document.getElementById("lb-count").textContent = data.length + " Ergebnis(se)";
        document.getElementById("lb-updated").textContent =
          "Aktualisiert: " + new Date().toLocaleTimeString();
      })
      .catch(function() {
        setMessage("api.asiai.dev nicht erreichbar");
      });
  }
