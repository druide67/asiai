...
<tbody id="lb-body">
<tr><td colspan="7" class="lb-loading">Loading community data...</td></tr>
</tbody>
</table>
  ...
  function fetchData() {
    ...
    fetch(url)
      ...
      .then(function(d) {
        ...
        render();
        document.getElementById("lb-count").textContent = data.length + " result(s)";
        document.getElementById("lb-updated").textContent =
          "Updated: " + new Date().toLocaleTimeString();
      })
      .catch(function() {
        setMessage("Could not reach api.asiai.dev");
      });
  }
