...
### 完璧主義の調査ループ (`asiai bench --instruct loop-search`)

単一ターンの IFEval と research-brief はこれらのモデルすべてで 100% に飽和する
ため、どちらも*完璧主義の調査ループ*を表面化させない: 曖昧で確認不能な検索結果を
受け入れず、no-progress のガードレールが停止させるまで意味的に等価なクエリを
再発行し続け、決して成果物を提供しないモデルである。`loop-search` のスイープ
（9 構成、M5、b9430、thinking on/off、2 つの曖昧さモード）がそれを切り分ける:

- **35B-A3B MoE は上限までループする** — **ベースと Qwopus ファインチューンの
  両方について、Q4 と Q8 のいずれでも**。より高い quant でも直らないため、この
  ループは量子化の作為ではなく **A3B MoE のアーキテクチャ由来**である。
- **dense 27B は決してループしない**（Q4 / Q5 / Q8）: 曖昧な結果を受け入れて
  ブリーフィングを書く。

したがってスループットのリーダー（MoE、約 118-123 t/s）とエージェント適性の
リーダー（dense 27B、約 25 t/s）は*異なるモデル*である。NousResearch の
Hermes Agent のようなハーネスにとっては、ループ耐性が生の decode を上回りうる —
最速のモデルが常に正しいエージェントとは限らない。（これはツールコールの結果
（MoE ファインチューンがより堅牢なエージェントだった）とは逆である: **適性は
失敗モードごとであり、したがって複数を計測すること。**）
