...
<tbody id="lb-body">
<tr><td colspan="7" class="lb-loading">Chargement des données communautaires...</td></tr>
</tbody>
</table>
  ...
  function fetchData() {
    ...
    fetch(url)
      ...
      .then(function(d) {
        ...
        render();
        document.getElementById("lb-count").textContent = data.length + " résultat(s)";
        document.getElementById("lb-updated").textContent =
          "Mis à jour : " + new Date().toLocaleTimeString();
      })
      .catch(function() {
        setMessage("Impossible de joindre api.asiai.dev");
      });
  }
