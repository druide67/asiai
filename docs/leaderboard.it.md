...
<tbody id="lb-body">
<tr><td colspan="7" class="lb-loading">Caricamento dati della community...</td></tr>
</tbody>
</table>
  ...
  function fetchData() {
    ...
    fetch(url)
      ...
      .then(function(d) {
        ...
        render();
        document.getElementById("lb-count").textContent = data.length + " risultato/i";
        document.getElementById("lb-updated").textContent =
          "Aggiornato: " + new Date().toLocaleTimeString();
      })
      .catch(function() {
        setMessage("Impossibile raggiungere api.asiai.dev");
      });
  }
