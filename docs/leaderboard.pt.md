...
<tbody id="lb-body">
<tr><td colspan="7" class="lb-loading">Carregando dados da comunidade...</td></tr>
</tbody>
</table>
  ...
  function fetchData() {
    ...
    fetch(url)
      ...
      .then(function(d) {
        ...
        render();
        document.getElementById("lb-count").textContent = data.length + " resultado(s)";
        document.getElementById("lb-updated").textContent =
          "Atualizado: " + new Date().toLocaleTimeString();
      })
      .catch(function() {
        setMessage("Não foi possível acessar api.asiai.dev");
      });
  }
