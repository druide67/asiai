...
- **Loaded-idle measurement** before the first timed run gives `idle_soc_watts` and `energy_per_token_active_j`.
- **`energy` block in the share payload**: base, rails, window, idle, conditions, all-or-nothing. Legacy GPU-rail fields are unchanged and tagged `power_scope: "gpu"`.
- **Gates `energy_provenance` and `energy_thermal`** refuse a figure rather than publish a smaller one.
- **Cards label the power rail** (`62W SoC · 1.35 J/tok`, or `20W GPU`); the agentic card gains a decode energy chip.
- **Leaderboard: Speed and Energy views** (SoC W, J/tok, J/tok active, idle W) in all nine locales; local dashboard gains SoC W and J/tok columns; `/metrics` gains `asiai_power_soc_watts`, `_dcs_watts`, `_dram_watts`.
- **`advisor --use-case efficiency`** ranks by measured J/token.
- **`mlx-vlm` engine adapter**; detection tells `mlx_vlm` from `mlx_lm`.
...
- **Agentic protocol `agentic-v5`**: the third prefix-test turn uses a prompt never served in the session. Phases that replay by design are exempt from the `session_replay` gate.
- **MTPLX version is read from the serving process** (`/health.mlx_runtime.path`); the Homebrew keg is a logged fallback.
- **IOReport: a rail that was not read is `None`, never `0`**; `soc_watts` is withheld when a required rail is missing. Monitoring withholds the legacy total the same way.
- **Docs**: metrics-spec 0.5.0 (M6/M7 marked GPU-rail legacy, M9–M13 added); methodology states the powermetrics validation covers the GPU rail only. A drift test pins docs to code.
...
- **Burst J/token uses `total − N` decode intervals** for N concurrent calls.
- **A sub-second energy window excludes that run** instead of refusing the engine's whole block.
- **Two cards saved by one process within a second no longer overwrite each other.**
- **Card text width is measured per glyph class**; a chip no longer overlaps the model name.
- **A model given as a path is shown by file name on cards.**
- **oMLX version lookup no longer queries PyPI.**
