...
(function () {
    ...
    function observedRate(key, target) {
        ...
        var now = Date.now();
        var w = c.win || (c.win = []);
        // Counter reset (engine restart): stale pre-restart highs would
        // hold the delta negative and blank the rate for a full window —
        // exactly when someone is watching a redeploy. Start fresh.
        if (w.length && target < w[w.length - 1].v) w.length = 0;
    ...
    function activityRows(node) {
        ...
        enginesOf(node).forEach(function (e) {
            ...
            if (typeof tokens === 'number' && tokens > 0) {
                // Keyed on the engine URL, not the display label: two
                // co-located llama.cpp instances both label "llamacpp"
                // when the lifecycle manifest is absent, and a shared key
                // would interleave their windows into one bogus rate.
                var tokKey = node.nickname + '/' + (e.url || engineLabel(e)) + '/tokens';
