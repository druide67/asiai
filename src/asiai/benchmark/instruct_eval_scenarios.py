...
VERIFIABLE_PROMPTS: list[dict] = [
    {"id": "words_min", "prompt": "Explain what a hash map is. Write at least 120 words.",
     "instructions": [{"type": "number_words", "args": {"min": 120}}]},
    {"id": "words_max", "prompt": "In 40 words or fewer, define recursion.",
     "instructions": [{"type": "number_words", "args": {"max": 40}}]},
    {"id": "sentences", "prompt": "Describe the water cycle in exactly 3 sentences.",
     "instructions": [{"type": "number_sentences", "args": {"exact": 3}}]},
    {"id": "paragraphs", "prompt": "Write about the bicycle's history in exactly 2 paragraphs.",
     "instructions": [{"type": "number_paragraphs", "args": {"exact": 2}}]},
    {"id": "bullets", "prompt": "List exactly 5 tips for better sleep as markdown bullet points.",
     "instructions": [{"type": "number_bullets", "args": {"exact": 5}}]},
    {"id": "sections", "prompt": "Write a short guide with exactly 3 markdown `##` sections.",
     "instructions": [{"type": "number_sections", "args": {"exact": 3}}]},
    {"id": "title", "prompt": "Write a one-paragraph movie pitch. Put a title wrapped in double "
     "angle brackets, like <<Title Here>>, at the top.",
     "instructions": [{"type": "title", "args": {}}]},
    {"id": "json", "prompt": "Output a JSON object with keys name, age, city and nothing else. "
     "No prose, no code fence.",
     "instructions": [{"type": "json_format", "args": {}}]},
    {"id": "choose", "prompt": "Is the Earth flat? Answer with exactly one word: Yes or No.",
     "instructions": [{"type": "choose_from", "args": {"options": ["Yes", "No"]}}]},
    {"id": "lowercase", "prompt": "Write a friendly greeting entirely in lowercase letters.",
     "instructions": [{"type": "all_lowercase", "args": {}}]},
    {"id": "uppercase", "prompt": "Write a short motivational slogan in ALL UPPERCASE.",
     "instructions": [{"type": "all_uppercase", "args": {}}]},
    {"id": "no_commas", "prompt": "Describe your ideal weekend without using any commas.",
     "instructions": [{"type": "no_commas", "args": {}}]},
    {"id": "end_phrase", "prompt": "Give one productivity tip and end your response with the exact "
     "phrase: That is all.",
     "instructions": [{"type": "end_phrase", "args": {"phrase": "That is all."}}]},
    {"id": "postscript", "prompt": "Recommend a book, then add a postscript starting with P.S.",
     "instructions": [{"type": "postscript", "args": {"marker": "P.S."}}]},
    {"id": "quotation", "prompt": "Reply with a single sentence wrapped entirely in double quotes.",
     "instructions": [{"type": "quotation", "args": {}}]},
    {"id": "kw_include", "prompt": "Write two sentences about gardening that mention the words "
     "soil and sunlight.",
     "instructions": [{"type": "keywords_include", "args": {"keywords": ["soil", "sunlight"]}}]},
    {"id": "kw_freq", "prompt": "Write a short paragraph about coffee that uses the word coffee "
     "at least 3 times.",
     "instructions": [{"type": "keyword_frequency", "args": {"keyword": "coffee", "min": 3}}]},
    {"id": "forbidden", "prompt": "Describe a sunset without using the words sun or orange.",
     "instructions": [{"type": "forbidden_words", "args": {"forbidden": ["sun", "orange"]}}]},
    {"id": "language_fr", "prompt": "Réponds en français : explique en deux phrases ce qu'est une "
     "API.",
     "instructions": [{"type": "response_language", "args": {"language": "fr"}}]},
    # multi-instruction prompts (prompt-level needs ALL to pass)
    {"id": "multi_format", "prompt": "Write a product blurb of at least 50 words, with a title in "
    ...
    {"id": "multi_constraints", "prompt": "List exactly 4 bullet points of travel advice, with no "
    ...
    {"id": "multi_lower_kw", "prompt": "In all lowercase, write three sentences about cats "
...
AGENTIC_SCENARIOS: dict[str, dict] = {
    "research-brief": {
        "prompt": RESEARCH_BRIEF_PROMPT, "topics": RESEARCH_TOPICS,
        "sections": RESEARCH_SECTIONS, "max_turns": INSTRUCT_MAX_TURNS,
    },
    "order-control": {
        "prompt": ORDER_CONTROL_PROMPT, "topics": RESEARCH_TOPICS,
        "sections": RESEARCH_SECTIONS, "max_turns": INSTRUCT_MAX_TURNS,
    },
    "research-brief-deep": {
        "prompt": RESEARCH_BRIEF_DEEP_PROMPT, "topics": RESEARCH_TOPICS_DEEP,
        "sections": RESEARCH_SECTIONS_DEEP, "max_turns": INSTRUCT_MAX_TURNS_DEEP,
    },
}
